...
void main(List<String> args) async {
  ...
  if (mode == 'mainnet') {
    var backendResult = await Process.run(
        'dfx', ['canister', 'id', 'greet__backend', '--network=ic']);
    var frontendResult = await Process.run(
        'dfx', ['canister', 'id', 'greet__frontend', '--network=ic']);
    ...
    var backendCanisterId = jsonContent['greet_backend'][mode];
    print('backendCanisterId: $backendCanisterId');

    var frontend_canister_id = jsonContent['greet_frontend'][mode];
...
export FRONTEND_CANISTER_ID=$frontend_canister_id
  ...
  outputFile = File(
      '$ROOT_DIRECTORY/src/flutter_project/lib/ICP/config.dart');
  await outputFile.writeAsString('''
